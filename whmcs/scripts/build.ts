...
const SOURCE_DIR = join(ROOT_DIR, "modules");
const DIST_DIR = join(ROOT_DIR, "dist");
const TEMP_DIR = join(ROOT_DIR, ".tmp-build");
const STAGE_DIR = join(TEMP_DIR, "modules");
...
function getVersion(): string {
  ...
  if (!version) {
    throw new Error("Version not found in whmcs/package.json");
  }
  return version;
}
...
function run(): void {
  if (!existsSync(SOURCE_DIR)) {
    throw new Error("modules directory not found in whmcs/");
  }

  const version = getVersion();
  const zipName = `zenocrypto-whmcs-v${version}.zip`;
  ...
  const zipResult = spawnSync("zip", ["-r", zipPath, "modules"], {
    cwd: TEMP_DIR,
    stdio: "inherit",
  });
